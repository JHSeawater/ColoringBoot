...
* **`GDD.md`** — 기획(무엇/왜)이자 **게임 규칙의 정본**: 규칙 §2 · 예시 스테이지(포도) §3 · 조작과 UI §6 · 기믹 후보 §7 · 레벨 제작 도구 §8 · 개발 기준 §10 · 출시 §11 · 미정 사항 §15. 기획자(사용자) 문서이므로 수정은 제안 → 승인 후에만.
...
* 무시해도 되는 로그: `Unable to join player connection multicast group (err: 10013)`(Windows 네트워크 권한 관련) · `Account API did not become accessible within 30 seconds`(`com.unity.ai.assistant` 패키지가 에디터 시작 때 계정 확인). 둘 다 작업과 무관.
...
버전 관리 (Git):
* 원격: `origin` = `https://github.com/JHSeawater/ColoringBoot.git` (`main`).
...
반대 방향 쌍은 1시↔7시, 3시↔9시, 5시↔11시 → 줄의 축은 3개다.
...
**스테이지 데이터:** 프로토타입 포맷 `{"name": ..., "cells": [[q, r, 시작 색, 목표 색], ...]}` (GDD §3).
...
**회귀 기준 — 포도 스테이지 (GDD §3):** 최소 풀이 5수, 그 5수의 순서 120가지 중 8가지만 성공. 위 규칙 서술로 재현됨을 확인했다(2026-09-24 시뮬레이션). 붓질·판정·솔버를 바꾸면 이 결과가 EditMode 테스트로 유지되어야 한다.

---

## 4. WebGL 제약 [CRITICAL] (GDD §10 · §11)

* **단일 스레드**: `Thread` · `Task.Run` 등 스레드를 쓰는 코드는 쓰지 않는다. 무거운 계산(솔버 BFS)은 에디터 도구로 돌리고, 런타임에 필요하면 여러 프레임에 나눠 실행한다(코루틴/`Awaitable`). 한 프레임에 오래 돌면 브라우저 탭이 멈춘다.
...
* **현황 (2026-09-24)**: WebGL Build Support 설치 확인(`list_build_targets`). 활성 빌드 타깃은 아직 Windows — WebGL 전환(`switch_build_target`, 전체 리임포트 발생)은 Task.md Phase 0에서 승인 후 진행.
...
* **입력**: 칸을 누른 채 끌기(터치·마우스 공통)가 기본. 보조 입력은 모바일 탭 → 여섯 방향 버튼, PC 방향키 + 숫자키. 되돌리기는 버튼 + Ctrl+Z. 멀티터치 시 **처음 닿은 손가락만** 추적하고, UI 위의 터치는 보드 입력으로 처리하지 않는다.
